...
export function WaveChart({ values }: WaveChartProps) {
  const width = 420;
  const height = 128;
  const points = linePoints(values, width, height);

  return (
    <svg className="chart chart-wave" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Pulse waveform">
      <path className="chart-grid" d="M12 32 H408 M12 64 H408 M12 96 H408" />
      <path className="chart-midline" d="M12 64 H408" />
      {points ? (
        <>
...
export function TrendChart({ history }: TrendChartProps) {
  ...
  return (
    <svg className="chart chart-trend" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Heart rate trend">
      <path className="chart-grid" d="M14 32 H406 M14 63 H406 M14 94 H406" />
      {points ? (
        <>
